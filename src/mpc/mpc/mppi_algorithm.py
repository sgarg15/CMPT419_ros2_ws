...
class MPPI:
    ...
    def rollout(self, observation: NDArray, actions: NDArray) -> NDArray:
        ...
        # =========================
        # STUDENT CODE START
        K, H = actions.shape[0], actions.shape[1]
        costs = np.zeros((K, H), dtype=float)
        for k in range(K):
            state = np.asarray(observation, dtype=float).reshape(3)
            prev_action = None
            for t in range(H):
                action = np.asarray(actions[k, t, :], dtype=float).reshape(self.act_dim)
                next_state, base_cost = self.dynamics_func(state, action)
                if t == 0:
                    cost_t = base_cost
                else:
    ...
    def get_action(self, observation: NDArray) -> NDArray:
        ...
        perturbed = np.clip(
            self.actions[np.newaxis, :, :] + eps,
            self.action_min,
            self.action_max,
        )
        costs = self.rollout(observation, perturbed)
        S = np.sum(costs, axis=1)
        S_min = np.min(S)
        w = np.exp(-(S - S_min) / self.temperature)
        w_bar = w / (np.sum(w) + self.epsilon)
        self.actions[:] = np.clip(
            np.average(perturbed, axis=0, weights=w_bar),
            self.action_min,
            self.action_max,
        )
        best_action = np.clip(
            np.asarray(self.actions[0], dtype=float).reshape(self.act_dim),
            self.action_min,
            self.action_max,
        )
        self.actions[:-1] = self.actions[1:]
        self.actions[-1] = self.actions[-2]  # duplicate last action
        # STUDENT CODE END
