#!/usr/bin/env python3
"""
PX4 bridge for attacker: subscribes to /game/attacker/velocity, publishes velocity
setpoints to /px4_2/fmu/in/*. Handles arm/offboard state machine and readiness checks.
"""
...
class Px4BridgeAttackerNode(Node):
    """Bridges game velocity commands to PX4 attacker (instance 2)."""

    def __init__(self):
        super().__init__("px4_bridge_attacker_node")

        sim_params = load_sim_params()
        bridge_cfg = sim_params.get("bridge", {})
        self._heartbeat_hz = float(bridge_cfg.get("heartbeat_hz", 20.0))
        self._setpoints_before_arm = int(
            bridge_cfg.get("arm_delay_sec", 1.0) * self._heartbeat_hz
        )
        ...
        self._target_system = 3  # MAV_SYS_ID for px4_instance=2
    ...
    def _get_effective_velocity(self):
        """Return (vx, vy, vz) in game frame. Zero if game over or no valid command."""
        if self._game_over:
            return (0.0, 0.0, 0.0)
    ...
    def _timer_cb(self):
        ...
        if not self._position_valid():
            if self._state != BridgeState.WAITING_POSITION:
                self.get_logger().info(
                    "Attacker: not ready — waiting for valid local position"
                )
            self._state = BridgeState.WAITING_POSITION
        else:
            if self._state == BridgeState.WAITING_POSITION:
                ...
                self._setpoint_counter = 0
                self.get_logger().info("Attacker: position valid, streaming setpoints")
        ...
        if self._state == BridgeState.STREAMING:
            ...
            if self._setpoint_counter >= self._setpoints_before_arm:
                ...
                self.get_logger().info("Attacker: engage offboard + arm")

        if self._state == BridgeState.ARMED_OFFBOARD and self._is_armed_offboard():
            self._state = BridgeState.ACTIVE
            self.get_logger().info(
                "Attacker: armed and offboard — now commanding velocity"
            )
        ...
        if self._state == BridgeState.ACTIVE and not self._velocity_received:
            self.get_logger().debug(
                "Attacker: no velocity command received yet",
